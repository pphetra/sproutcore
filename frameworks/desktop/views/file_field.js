...
/** @class
  ...
  Implements a customized file input by creating a transparent file type input over top of a SC.ButtonView and using a hidden iframe to receive results.
...
/** @scope SC.FileView.prototype */
{
  ...
  /**
      The placeholder text beside the button when the input has no selection.
  ...
  _iframeLoad: function(evt) {
    var result = null;
    try {
        result = this._iframe.$()[0].contentWindow.document.body;
    } catch(err) {
        result = this._iframe.$()[0].contentDocument.body;
    }
